...
MAX_CHARS = 1200  # ~300 tokens hard ceiling
...
class FetchArticleTool(BaseTool):
    ...
    Fetches and cleans article text from a URL (hard-truncated to 1,200 chars).
    """

    name: str = "fetch_article"
    description: str = (
        "Fetches and extracts the main clean text content from a web article URL. "
        "Content is hard-truncated to 1,200 characters. "
    ...
    def _run(self, url: str) -> str:
        """
        Execute article fetch with 1,200 char hard ceiling inside the tool.
        """
        ...
        try:
            ...
            if status == "blocked_untrusted_domain":
                ...
                return f"Unable to fetch article from '{url}': {domain_msg} Select a whitelisted URL."
            elif status == "blocked_by_robots":
                return f"Unable to fetch article from '{url}': Scraping disallowed by robots.txt."
            elif status == "error":
                err_text = result.get("text", "Network error.")
                return f"Error fetching article from '{url}': {err_text}"
            ...
            if not text:
                return f"Article at '{url}' returned empty text."

            # Enforce 1,200 char (~300 tokens) hard ceiling inside the tool
            text = text[:MAX_CHARS]
